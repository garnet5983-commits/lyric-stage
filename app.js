...
const fonts=window.LyricFonts.ids;
const fontOptions=(global=false)=>`${global?'':'<option value="global">全体設定</option>'}${['bold','serif','rounded','mono'].map((id,i)=>`<option value="${id}">${['太ゴシック','明朝','丸ゴシック','モノ'][i]}</option>`).join('')}${window.LyricFonts.list.map(f=>`<option value="${f.id}">${f.name}</option>`).join('')}${global?'<option value="random">ランダム</option>':''}`;
$('globalFont').innerHTML=fontOptions(true);
const geometry=window.LyricGeometry;
function chosenFont(p,i){const g=$('globalFont').value;const picks=['web14','web23','web0','web15','web10','web27','serif','bold'];return p.font==='global'?(g==='random'?picks[Math.floor(seeded(i+21)*picks.length)]:g):p.font}
async function prepareFonts(){const content=state.phrases.map(p=>p.text).join('');const ids=new Set(state.phrases.map(chosenFont));for(const id of ids){if(!id.startsWith('web'))continue;setStatus(`フォントを準備しています：${window.LyricFonts.list.find(f=>f.id===id)?.name||id}`);const full=await window.LyricFonts.load(id,content);if(!full)throw Error('フォントに必要な文字をオフラインで取得できません。接続してから再試行してください。')}draw()}
function queueFont(p,i){const id=chosenFont(p,i);if(!id.startsWith('web'))return;window.LyricFonts.load(id,state.phrases.map(x=>x.text).join('')).then(draw).catch(()=>setStatus('追加フォントを取得できません。通信状態を確認してください。'))}
...
function randomStyle(i,phrase){
 ...
 return {effect,font:'global',color:'',pattern:geometry.ids[i%geometry.ids.length]};
}
...
function draw(time=ui.audio.currentTime,forcedIndex=-1,preview=false){
 ...
 const pattern=geometry.names[p.pattern]?p.pattern:'none';
 if(pattern==='stencil'){geometry.stencil({ctx,canvas:ui.canvas,phrase:p,time:t,themeName:$('visualTheme').value,font});return}
 geometry.render({ctx,canvas:ui.canvas,phrase:p,time:t,pulse,themeName:$('visualTheme').value,intensity:Number($('intensity').value),pattern});
 if(pattern==='grid'&&geometry.typeGrid({ctx,canvas:ui.canvas,phrase:p,time:t,pulse,font,color:$('globalColor').value,themeName:$('visualTheme').value}))return;
...
function previewEffect(i){
 ...
 const start=performance.now(),phrase=state.phrases[i];queueFont(phrase,i);
 function step(now){
...
$('ratio').onchange=resize;for(const id of ['globalFont','globalColor','fontSize','dim','visualTheme','intensity'])$(id).addEventListener('input',draw);$('globalFont').addEventListener('change',()=>{state.phrases.forEach(queueFont)});
